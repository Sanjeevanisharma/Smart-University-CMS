...
const getStudents = asyncHandler(async (req, res) => {
  ...
  if (req.accepts('json')) return res.json(students);
...
const getStudent = asyncHandler(async (req, res) => {
  ...
  if (!student) {
    if (req.accepts('json')) return res.status(404).json({ message: 'Student not found' });
    req.flash('error', 'Student not found');
    return res.redirect('/students');
  }
  if (req.accepts('json')) return res.json(student);
...
const createStudent = asyncHandler(async (req, res) => {
  ...
  if (exists) {
    if (req.accepts('json')) return res.status(400).json({ message: 'Email or Roll Number already exists' });
  ...
  if (createLogin === 'on') {
    ...
    if (userExists) {
      if (req.accepts('json')) return res.status(400).json({ message: 'A user with this email already exists' });
      req.flash('error', 'A user with this email already exists');
      return res.redirect('/students/new');
    }
    if (!password || password.length < 6) {
      if (req.accepts('json')) return res.status(400).json({ message: 'Password must be at least 6 characters' });
  ...
  if (req.accepts('json')) return res.status(201).json(created);
...
const updateStudent = asyncHandler(async (req, res) => {
  ...
  if (req.accepts('json')) return res.json(updated);
...
const deleteStudent = asyncHandler(async (req, res) => {
  ...
  if (req.accepts('json')) return res.json({ message: 'Student removed' });
